...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const [userRole, setUserRole] = useState('admin_super')
  ...
  const navigation = roleNav[userRole] || roleNav.admin_super
  const isActive = (href: string) => pathname === href || pathname.startsWith(href + '/')

  return (
    <div className="min-h-screen bg-gray-50">
      {sidebarOpen && <div className="fixed inset-0 z-40 bg-black/40 lg:hidden backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />}
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-[#0a1628] flex flex-col transition-transform duration-300 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-16 items-center justify-between px-5 border-b border-white/10 shrink-0">
          <Link href="/dashboard" className="flex items-center gap-3">
            ...
            <div><div className="text-base font-bold leading-none text-white tracking-tight">UNIG</div><div className="text-[10px] uppercase tracking-widest text-gray-500 leading-none mt-0.5">SGPI</div></div>
          </Link>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
          {navigation.map(item => (
            <Link key={item.href} href={item.href} onClick={() => setSidebarOpen(false)}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${isActive(item.href) ? 'bg-[#4fc3f7]/10 text-[#4fc3f7]' : 'text-gray-400 hover:bg-white/5 hover:text-gray-200'}`}>
              <item.icon className="h-5 w-5 shrink-0" />{item.name}</Link>
          ))}
        </nav>
        <div className="border-t border-white/10 p-4 shrink-0">
          <div className="flex items-center gap-3 px-2 py-2">
            ...
            <button onClick={handleLogout} className="p-1.5 rounded-lg text-gray-500 hover:bg-white/10 hover:text-red-400 transition-colors" title="Sair"><LogOut className="h-4 w-4" /></button>
          </div>
        </div>
      </aside>
        ...
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-gray-200">
          <div className="flex items-center justify-between h-16 px-4 lg:px-8">
            <div className="flex items-center gap-4">
              <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-2 -ml-2 rounded-lg text-gray-500 hover:bg-gray-100"><Menu className="h-5 w-5" /></button>
              <div><h1 className="text-lg font-bold text-gray-900 leading-tight">SGPI</h1><p className="text-xs text-gray-500 hidden sm:block">Sistema de Gestão de Projetos Integradores</p></div>
            </div>
            <div className="flex items-center gap-3">
              <button className="p-2 rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-600 relative"><Bell className="h-5 w-5" /></button>
